 ...
 * 5. Cab / Taxi (Uber / Ola)
 * 
 * For inter-city (> 50 km):
 * 1. Express Train (IRCTC / Vande Bharat / Shatabdi)
 * 2. Outstation AC Cab (Uber Intercity / Ola Outstation)
 * 3. Inter-City AC Bus (Volvo / RedBus / State Roadways)
 * 4. Bike Taxi (Unavailable for inter-city)
 * 5. Auto-Rickshaw (Unavailable for inter-city)
 */
export const calculateMultiModalEstimates = (distanceKm, cityName = '') => {
  const d = Math.max(0.5, Number(distanceKm) || 1);
  const isInterCity = d > 50;
  const hasMetro = !isInterCity && hasMetroNetwork(cityName);

  let rawModes = [];

  if (isInterCity) {
    // 1. Inter-City Express Train (IRCTC / Vande Bharat)
    const trainFare = Math.round(d * 2.2 + 80);
    const trainDuration = Math.max(30, Math.round((d / 75) * 60 + 20));
    const trainCo2 = Math.round(d * 18);

    // 2. Outstation AC Cab (Uber Intercity / Ola)
    const cabFare = Math.round(500 + 13 * d);
    const cabDuration = Math.max(35, Math.round((d / 65) * 60));
    const cabCo2 = Math.round(d * 130);

    // 3. Inter-City AC Bus (Volvo / RedBus / State Transport)
    const busFare = Math.round(60 + 1.8 * d);
    const busDuration = Math.max(45, Math.round((d / 50) * 60 + 25));
    const busCo2 = Math.round(d * 25);

    rawModes = [
      {
        id: 'train',
        name: 'Express Train (IRCTC / Vande Bharat)',
        icon: 'Train',
        emoji: '🚆',
        estimatedCost: trainFare,
        durationMinutes: trainDuration,
        speedKmh: 75,
        co2Grams: trainCo2,
        capacity: 'AC Chair Car / Sleeper',
        available: true,
        bestFor: 'Fastest & most comfortable inter-city travel avoiding highway traffic',
        bookingUrl: 'https://www.irctc.co.in/',
        bookingLabel: 'Book on IRCTC'
      },
      {
        id: 'outstation-cab',
        name: 'Outstation AC Cab (Uber Intercity / Ola)',
        icon: 'CarTaxiFront',
        emoji: '🚗',
        ...
        speedKmh: 65,
        co2Grams: cabCo2,
        capacity: 'Up to 4 Passengers + Bags',
        available: true,
        bestFor: 'Door-to-door comfort, family travel, and flexible stops on highway',
        bookingUrl: 'https://m.uber.com/',
        bookingLabel: 'Book Outstation Uber'
      },
      {
        id: 'intercity-bus',
        name: 'Inter-City AC Bus (Volvo / RedBus)',
        icon: 'Bus',
        emoji: '🚌',
        ...
        speedKmh: 50,
        co2Grams: busCo2,
        capacity: 'AC Sleeper / Seater',
        available: true,
        bestFor: 'Budget-friendly highway travel with frequent departures',
      ...
      },
      {
        ...
        available: false,
        unavailableNotice: `Bike taxis operate only within urban limits (< 50 km). Inter-city route (${d} km) is not supported.`,
        bestFor: 'Local short trips',
        bookingUrl: null,
        bookingLabel: 'Not Available for Inter-City'
      },
      {
        id: 'auto',
        ...
        available: false,
        unavailableNotice: `Auto-rickshaws operate only locally within city borders (< 40 km).`,
        bestFor: 'Local city hops',
        bookingUrl: null,
        bookingLabel: 'Not Available for Inter-City'
      }
    ];
  } else {
    // Intra-City Transit (<= 50 km)
    // 1. Bike Taxi / Rapido
  ...
  const minCost = activeModes.length > 0 ? Math.min(...activeModes.map((m) => m.estimatedCost)) : 0;
  const minTime = activeModes.length > 0 ? Math.min(...activeModes.map((m) => m.durationMinutes)) : 0;
  const minCo2 = activeModes.length > 0 ? Math.min(...activeModes.map((m) => m.co2Grams)) : 0;
